			...
			if ( !$is_preview ) {
				...
				echo '<div class="iframe" style="'.implode(';',$iframe_css).'">';
					...
					echo '<div class="iframe-accept">';
						$accept_txts = array(
							__( 'By playing this video, you accept the', 'wpreform' ),
							__( 'general conditions of use', 'wpreform' ),
							__( 'from', 'wpreform' ),
						);
						printf( __('<p class="iframe-accept--txt">'.$accept_txts[0].' <a class="iframe-accept-cgu-link" href="%1s" target="blank" rel="noreferrer">'.$accept_txts[1].'</a> '.$accept_txts[2].' <strong>%2s</strong>.</p>'), $providers[$datas->provider_name], $datas->provider_name );
